...
> **UI/UXデザインの詳細は [DESIGN.md](DESIGN.md) を参照。**
> Dark Neumorphism (Soft UI Dark) スタイルを採用。

画面のワイヤーフレーム、コンポーネント仕様、カラーパレット、アニメーションは **[DESIGN.md](DESIGN.md)** を参照。

#### 画面一覧

| 画面 | 主な機能 |
|------|---------|
| **Home** | mDNS自動検出デバイス一覧、手動IP:Port接続、接続履歴 |
| **Monitor** | リアルタイムログ表示、フィルター、一時停止、コピー、保存、コマンド送信 |
| **Settings** | 表示設定、接続設定、ログ設定、About |
```
