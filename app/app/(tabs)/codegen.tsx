...
const BOARDS: BoardDef[] = [
  {
    ...
    platform: 'arduino',
    needsWifi: true,
    description: 'ESP8266/32, Nano ESP32, M5Stack, XIAO',
    ideSetup: 'NodeMCU / ESP32 Dev Module',
  },
  {
    id: 'arduino-r4-wifi',
    name: 'UNO R4 WiFi',
...
function generateCode(boardId: string, ssid: string, password: string, security: SecurityOption = 'none', devicePassword: string = ''): string {
  switch (boardId) {
    ...
    case 'arduino-r4-wifi':
      return `/**
 * Serial Air — Arduino UNO R4 WiFi Test
 * Board: Arduino UNO R4 WiFi
 * IDE: Arduino IDE → Board: "Arduino UNO R4 WiFi"
 *
 * Uses WirelessSerial library with WiFiS3 (auto-detected).
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("=== Serial Air — UNO R4 WiFi ===");
...
void loop() {
    ...
    if (millis() - last > 2000) {
        ...
        output->print("analog:"); output->print(a0);
        output->print(" voltage:"); output->println(voltage);
    }
}`;
...
export default function CodeGeneratorScreen() {
  const [boardId, setBoardId] = useState('esp');
